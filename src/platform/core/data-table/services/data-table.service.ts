...
@Injectable()
export class IgDataTableService {
   ...
   * - data: any[]
   * - sortBy: string
   * - sortOrder: Direction = Direction.Ascending
   *
   * Sorts [data] parameter by [sortBy] and [sortOrder] and returns the sorted data.
   */
  sortData(data: any[], sortBy: string, sortOrder: Direction = Direction.Ascending): any[] {
    if (sortBy) {
      data.sort((a: any, b: any) => {
        ...
        return direction * (sortOrder === Direction.Descending ? -1 : 1);
      });
    }
    return data;
  }
