...
`ig-message` element generates an inline message with an icon, color, label and sublabel.
...
```html
<ig-message #message label="Label" sublabel="Sublabel goes here" icon="warning" color="primary | blue | red" [opened]="true">
  <button ig-message-actions md-button>View More<button>
  <button ig-message-actions md-button (click)="message.close()">Close<button>
  // .. body goes here
</ig-message>  
```
