## IgFileInputComponent: ig-file-input
...
```html
<ig-file-input [(ngModel)]="files" color="primary" (select)="selectEvent($event)"
...
| clear | function() | Used to clear the selected files from the [IgFileInputComponent].
...
## IgFileSelectDirective: igFileSelect
...
```html
<input type="file" igFileSelect (fileSelect)="files = $event" multiple>
```
...
## IgFileDropDirective: igFileDrop
...
```html
<div igFileDrop (fileDrop)="files = $event" multiple [disabled]="disabled">
</div> 
```
